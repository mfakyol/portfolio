...
export default function Aurora({
  ...
  const settings = useRef({ colorStops, amplitude, blend, speed });
  settings.current = { colorStops, amplitude, blend, speed };
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL — the page's CSS gradient background is fine on its own.
      return;
    }
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uTime.value = t * 0.001 * (s.speed ?? 0.4);
      program.uniforms.uAmplitude.value = s.amplitude ?? 1.0;
      program.uniforms.uBlend.value = s.blend ?? 0.5;
      program.uniforms.uColorStops.value = (s.colorStops ?? colorStops).map(toRGB);
